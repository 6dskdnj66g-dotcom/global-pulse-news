...
import { Lock, Mail, User, Apple, Loader2 } from 'lucide-react';
...
const SignupPage: React.FC = () => {
    ...
    const handleGoogleLogin = async () => {
        ...
        if (result.success) {
            ...
            setError(result.error || (isRtl ? 'فشل التسجيل بجوجل' : 'Google sign-up failed'));
        }
        setLoading(false);
    };
    ...
    return (
        <Layout>
            <SEO title={isRtl ? 'إنشاء حساب' : 'Sign Up'} />
                ...
                <div className="max-w-md w-full space-y-8 bg-paper dark:bg-slate-900 p-8 rounded-2xl shadow-2xl border border-black/10 dark:border-white/10">
                    ...
                    <div className="text-center">
                        ...
                        <p className="mt-2 text-sm text-muted">
                            {isRtl ? 'انضم إلينا واحصل على آخر الأخبار' : 'Join us and get the latest news'}
                        </p>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            ...
                            <span>{isRtl ? 'التسجيل بـ Google' : 'Sign up with Google'}</span>
                        </button>
                        ...
                        >
                            <Apple size={20} />
                            <span>{isRtl ? 'التسجيل بـ Apple' : 'Sign up with Apple'}</span>
                        </button>
                    </div>
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit}>
                        <div className="relative">
                            <User className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-3.5 text-muted`} size={18} />
                            <input
                                type="text"
                                required
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2"
                        >
                            {loading ? (
